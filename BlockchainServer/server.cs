...
public class serv
{
	public static void Main()
	{
		...
		int amountOfBlocks = Directory.GetFiles("D:\\Code\\DC-Cryptocurrency\\BlockchainServer\\blockchain\\", "*.*", SearchOption.TopDirectoryOnly).Length;
		while (true)
		{
			try
			{
				...
				{
					if (Directory.GetFiles("D:\\Code\\DC-Cryptocurrency\\BlockchainServer\\blockchain\\", "*.*", SearchOption.TopDirectoryOnly).Length > 0)
					{
				...
				{
					if (Directory.GetFiles("D:\\Code\\DC-Cryptocurrency\\BlockchainServer\\pendingblocks\\", "*.*", SearchOption.TopDirectoryOnly).Length > 0)
					{
				...
				{
					StreamReader readAll = new StreamReader("D:\\Code\\DC-Cryptocurrency\\BlockchainServer\\blockchain\\block" + (c+1) + ".txt");
				...
				if(File.Exists("D:\\Code\\DC-Cryptocurrency\\BlockchainServer\\pendingblocks\\block" + (amountOfBlocks + 1) + ".txt"))
				{
					readCurrentBlock = new StreamReader("D:\\Code\\DC-Cryptocurrency\\BlockchainServer\\pendingblocks\\block" + (amountOfBlocks + 1) + ".txt");
					lasthashget = new StreamReader("D:\\Code\\DC-Cryptocurrency\\BlockchainServer\\blockchain\\block" + (amountOfBlocks) + ".txt");
				...
				Console.WriteLine("\nCreated new as:" + finalHash + "\n");
				StreamWriter sw = new StreamWriter("D:\\Code\\DC-Cryptocurrency\\BlockchainServer\\pendingblocks\\block" + (amountOfBlocks+1) + ".txt");
				if (hashHistory != null)
				{
	...
	static void GetBlockchain(int whichBlock, TcpListener myList, Socket s)
	{
		...
		StreamReader readAll = new StreamReader("D:\\Code\\DC-Cryptocurrency\\BlockchainServer\\blockchain\\block" + whichBlock + ".txt");
	...
	{
		string[] everyTransaction = new string[4000];
		StreamReader readAll = new StreamReader("D:\\Code\\DC-Cryptocurrency\\BlockchainServer\\pendingblocks\\block" + whichBlock + ".txt");
	...
	{
		int chainLength = Directory.GetFiles("D:\\Code\\DC-Cryptocurrency\\BlockchainServer\\blockchain\\", "*.*", SearchOption.TopDirectoryOnly).Length;
		ASCIIEncoding sendbak = new ASCIIEncoding();
	...
	{
		int chainLength = Directory.GetFiles("D:\\Code\\DC-Cryptocurrency\\BlockchainServer\\blockchain\\", "*.*", SearchOption.TopDirectoryOnly).Length;
		int pendingLength = Directory.GetFiles("D:\\Code\\DC-Cryptocurrency\\BlockchainServer\\pendingblocks\\", "*.*", SearchOption.TopDirectoryOnly).Length;
		ASCIIEncoding sendbak = new ASCIIEncoding();
	...
	static void SubmitMine(int noncey, string rewardee, int blockNum, TcpListener myList, Socket s)
	{
		...
		{
			//Reads current block
			StreamReader readCurrentBlock = new StreamReader("D:\\Code\\DC-Cryptocurrency\\BlockchainServer\\pendingblocks\\block" + (blockNum) + ".txt");
			...
			else
			{
				...
				ASCIIEncoding sendbak = new ASCIIEncoding();
				s.Send(sendbak.GetBytes("\nMined new block: " + finalHash + " : " + noncey));
				StreamWriter sw = new StreamWriter("D:\\Code\\DC-Cryptocurrency\\BlockchainServer\\blockchain\\block" + (blockNum) + ".txt");
				File.Delete("D:\\Code\\DC-Cryptocurrency\\BlockchainServer\\pendingblocks\\block" + (blockNum) + ".txt");
				sw.Write(lastHash + "\n" + finalHash + "\n" + noncey + "\n" + hashHistory);
