...
$pendingBlockchain = './pendingblockchain/';
...
function submitBlock()
{
	...
	if ($_FILES["file"]["error"] == UPLOAD_ERR_OK)
	{
			...
			if(file_exists($uploads_dir . "/" . $name))
			{
				...
				$cost = floatval(trim(file('./programs/' . $_GET['programID'] . ".cfg")[4])) * ($_GET['time'] / 60.0 / floatval(trim(file('./programs/' . $_GET['programID'] . ".cfg")[3])));
				
				$contents = trim(file('./programs/' . $_GET['programID'] . ".cfg")[0]) . PHP_EOL . trim(file('./programs/' . $_GET['programID'] . ".cfg")[1]) . PHP_EOL . trim(file('./programs/' . $_GET['programID'] . ".cfg")[2]) . PHP_EOL . floatval(trim(file('./programs/' . $_GET['programID'] . ".cfg")[3])) - $_GET['time'] / 60.0 . PHP_EOL .trim(file('./programs/' . $_GET['programID'] . ".cfg")[4]) . PHP_EOL .trim(file('./programs/' . $_GET['programID'] . ".cfg")[5]); 
				file_put_contents('./programs/' . $_GET['programID'] . ".cfg", $contents, LOCK_EX);
...
function getProgramLifeLeft()
{
	...
	try
	{
		$timeLeft = trim(file("./programs/" . $_GET['programID'] . ".cfg")[3]);
		echo $timeLeft;
	}
	catch (Exception $e)
	{
...
function assignProgram()
{
	...
	for ($x = 0; $x < count($programs); $x++)
	{
		...
		{
		$life = floatval(trim(file("./programs/" . $programs[$x])[3]));
